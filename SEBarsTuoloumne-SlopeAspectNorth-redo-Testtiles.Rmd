---
title: "SE Bars Notebook - Tuoloumne Test tiles slope aspect northness"
...
library(raster)
library(tidyverse)
aso_tuol <- raster('data/ASO_3M_SD_USCATE_20180528_binary_wgs84.tif')
...
rastlist <- list.files(path = "data/SLASNORNORMBASEREDOTUOL/", pattern='.tif', all.files=TRUE, full.names=TRUE)
 library(caret)
i=0
plo <- data.frame()
...
write_csv(df_tuol,file='./data/SLASNORNORMBASEREDOTUOL/processed-data.csv')
...
print(p)
ggsave("figs/all_scenes_tuoloumne-testtiles-slopeaspectnorthness.png",dpi=300, dev='png',  height=8, width=10,units="in")
# Finished bar plot
