...
df_ch_demextsrtmdem <- read_csv(file='./data/CHDEMPLUSSRTM30M/processed-data.csv')
df_ch_demextsrtmdem$demresolution <- '30m - SRTM'
...
ggsave(filename = "figs/tests-dem-ch-rev3-30m.png",dpi = 300,height=4,width=8,units="in")
```
...
df_co_demextsrtmdem <- read_csv(file='./data/CODEMPLUSSRTM30M/processed-data.csv')
df_co_demextsrtmdem$demresolution <- '30m - SRTM'
...
perfdemres_co %>% mutate(score = round(avg_metric,2)) %>% 
 ...
 labs(x="" , y="",subtitle="",fill="") + theme_minimal(base_size = 18)+
  ggpubr::theme_pubclean()
ggsave(filename = "figs/tests-dem-co-rev2-30m.png",dpi = 300,height=4,width=8,units="in")
```
...
ggsave("figs/resolutionanalysis-rev2-30m.png",dpi=300, dev='png',  height=8, width=12,units="in")
```
