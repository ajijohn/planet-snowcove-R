---
title: "SE Bars Notebook - CO with BASE and CHM REDO"
...
rastlist <- list.files(path = "data/BASECHMREDOCO/", pattern='.tif', all.files=TRUE, full.names=TRUE)
library(caret)
i=0
plo <- data.frame()
skip_to_next <- FALSE
...
write_csv(df_gunni,file='./data/BASECHMREDOCO/processed-data.csv')
...
print(p)
ggsave("figs/all_scenes_gunnison_cobasechmredo.png",dpi=300, dev='png',  height=8, width=10,units="in")
# Finished bar plot
...
#   theme_classic() +
#   scale_fill_manual(values=c('#999999','#E69F00'))
ggsave("figs/all_scenes_gunnison_specific-basewithchmredo.png",dpi=300, dev='png',  height=8, width=10,units="in")
```

plot the studied area

```{r }
ostudyarea <- raster('data/BASECHMREDOCO/20180524_172637_0f2d_3B_AnalyticMS_SR_clip_merged.tif')
...
ggsave(filename = "figs/scenepreditCO_both_rev_BASECHMredo.png",dpi = 300,height=4,width=8,units="in")
...
ggsave("figs/co_aoi_original_ms-BASECHMredo.png",dpi=300, dev='png',  height=8, width=12,units="in")
```
...
write_csv(df_gunni,file='./data/BASECHMREDOCO/processed-data_ch_OA.csv')
```
...
write_csv(df_gunni,file='./data/BASECHMREDOCO/processed-data_ch_FA.csv')
```
...
write_csv(df_gunni,file='./data/BASECHMREDOCO/processed-data_ch_DENSE.csv')
```
...
write_csv(df_gunni,file='./data/BASECHMREDOCO/processed-data_ch_MEDIUM.csv')
```
...
write_csv(df_gunni,file='./data/BASECHMREDOCO/processed-data_ch_SPARSE.csv')
```
...
write_csv(df_gunni,file='./data/BASECHMREDOCO/processed-data_ch_UC.csv')
```
...
write_csv(df_gunni,file='./data/BASECHMREDOCO/processed-data_ch_CE.csv')
```
...
write_csv(df_gunni,file='./data/BASECHMREDOCO/processed-data_ch_OPEN.csv')
```
