...
df <- raster::stack('data/newstudy-swiss-scene1.tif')
...
ggsave(filename = "figs/swiss_canopyheightmap-complete-nov2021.png",dpi = 300,height=4,width=8,units="in")
```
...
high_veg <- datafra_comb_filtered %>% dplyr::filter(!is.na(veg_height)) %>% dplyr::filter(veg_height > 1 ) %>% as.data.frame()
...
low_veg <- datafra_comb_filtered %>%  dplyr::filter(veg_height <= 1 ) %>% as.data.frame()
...
bare_veg <- datafra_comb_filtered %>%  dplyr::filter(veg_height < 1 | veg_height ==1 ) %>% as.data.frame()
...
yes_veg <- datafra_comb_filtered %>%  dplyr::filter(veg_height > 1 ) %>% as.data.frame()
