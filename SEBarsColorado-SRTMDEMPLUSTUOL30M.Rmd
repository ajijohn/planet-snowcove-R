---
title: "SE Bars Notebook - CO with DEMPLUS SRTM Tuol only new 30M"
...
rastlist <- list.files(path = "data/CODEMPLUSSRTM30M/", pattern='.tif', all.files=TRUE, full.names=TRUE)
library(caret)
i=0
plo <- data.frame()
skip_to_next <- FALSE
...
rastlist <- rastlist[! rastlist %in% c('data/CODEMPLUSSRTM30M//20180524_172148_103d_3B_AnalyticMS_SR_clip_merged.tif')]
...
write_csv(df_gunni,file='./data/CODEMPLUSSRTM30M/processed-data.csv')
...
print(p)
ggsave("figs/all_scenes_gunnison_srtmtuol30m.png",dpi=300, dev='png',  height=8, width=10,units="in")
# Finished bar plot
...
```{r }
library(tidyverse)
plotch_longer_df <- read_csv(file='./data/CODEMPLUSSRTM30M/processed-data.csv')
...
print(pch)
ggsave("figs/all_scenes_gunnison_srtmtuolonly30m.png",dpi=300, dev='png',  height=8, width=10,units="in")
