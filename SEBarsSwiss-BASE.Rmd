---
title: "SE Bars Notebook- Swiss BASE"
...
rastlist <- list.files(path = "data/CHBASE/", pattern='.tif', all.files=TRUE, full.names=TRUE)
 library(caret)
i=0
ploch <- data.frame()
...
write_csv(df_swiss,file='./data/CHBASE/processed-data.csv')
...
print(pch)
ggsave("figs/all_scenes_ch-BASE.png",dpi=300, dev='png',  height=8, width=10,units="in")
# Finished bar plot
...
```{r }
library(tidyverse)
plotch_longer_df <- read_csv(file='./data/CHBASE/processed-data.csv')
...
write_csv(df_gunni,file='./data/CHBASE/processed-data_ch_OA.csv')
```
...
write_csv(df_gunni,file='./data/CHBASE/processed-data_ch_FA.csv')
```
...
write_csv(df_gunni,file='./data/CHBASE/processed-data_ch_DENSE.csv')
```
...
write_csv(df_gunni,file='./data/CHBASE/processed-data_ch_MEDIUM.csv')
```
...
write_csv(df_gunni,file='./data/CHBASE/processed-data_ch_SPARSE.csv')
```
...
write_csv(df_gunni,file='./data/CHBASE/processed-data_ch_UC.csv')
```
...
write_csv(df_gunni,file='./data/CHBASE/processed-data_ch_CE.csv')
```
...
write_csv(df_gunni,file='./data/CHBASE/processed-data_ch_OPEN.csv')
```
