---
title: "SE Bars Notebook - CH with DEMPLUS SRTM Tuol only30M"
...
```{r , echo=FALSE}
library(raster)
aso_colorado <- raster('data/EUCHDB20170517_SUPERsnow_depth_3m_binary_wgs84_3m.tif')
...
rastlist <- list.files(path = "data/CHDEMPLUSSRTM30M/", pattern='.tif', all.files=TRUE, full.names=TRUE)
library(caret)
i=0
plo <- data.frame()
skip_to_next <- FALSE
...
write_csv(df_gunni,file='./data/CHDEMPLUSSRTM30M/processed-data.csv')
...
print(p)
ggsave("figs/all_scenes_ch_srtmtuol30m.png",dpi=300, dev='png',  height=8, width=10,units="in")
# Finished bar plot
...
```{r }
library(tidyverse)
plotch_longer_df <- read_csv(file='./data/CHDEMPLUSSRTM30M/processed-data.csv')
...
print(pch)
ggsave("figs/all_scenes_ch_srtmtuolonly30m.png",dpi=300, dev='png',  height=8, width=10,units="in")
