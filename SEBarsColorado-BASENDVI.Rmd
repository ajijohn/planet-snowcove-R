---
title: "SE Bars Notebook - CO with BASE and NDVI"
...
rastlist <- list.files(path = "data/COBASENDVI/", pattern='.tif', all.files=TRUE, full.names=TRUE)
library(caret)
i=0
plo <- data.frame()
skip_to_next <- FALSE
...
write_csv(df_gunni,file='./data/COBASENDVI/processed-data.csv')
...
#   theme_classic() +
#   scale_fill_manual(values=c('#999999','#E69F00'))
ggsave("figs/all_scenes_gunnison_specific-basewithndvi.png",dpi=300, dev='png',  height=8, width=10,units="in")
```

plot the studied area

```{r }
ostudyarea <- raster('data/COBASENDVI/20180524_172637_0f2d_3B_AnalyticMS_SR_clip_merged.tif')
...
ggsave(filename = "figs/scenepreditCO_both_rev_BASENDVI.png",dpi = 300,height=4,width=8,units="in")
...
ggsave("figs/co_aoi_original_ms-BASENDVI.png",dpi=300, dev='png',  height=8, width=12,units="in")
```
