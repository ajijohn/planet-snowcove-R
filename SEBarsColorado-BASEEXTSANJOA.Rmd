---
title: "SE Bars Notebook - CO with BASE with Extension San Joaquin"
...
rastlist <- list.files(path = "data/COLORADOBASEEXT2/", pattern='.tif', all.files=TRUE, full.names=TRUE)
library(caret)
i=0
plo <- data.frame()
skip_to_next <- FALSE
...
write_csv(df_gunni,file='./data/COLORADOBASEEXT2/processed-data.csv')
...
print(p)
ggsave("figs/all_scenes_gunnison_cobaseandextsqn.png",dpi=300, dev='png',  height=8, width=10,units="in")
# Finished bar plot
...
```{r }
scenes_study <- 
  c('20180524_172142_103d', '20180524_172143_103d', '20180524_172144_103d', '20180524_172145_103d', '20180524_172326_0f51', '20180524_172327_0f51', '20180524_172329_0f51', '20180524_172330_0f51', '20180524_172331_0f51', '20180524_172634_0f2d', '20180524_172635_0f2d', '20180524_172637_0f2d')
 plot_longer_df %>% mutate(fshort = tolower(substr(plot_longer_df$filename,start = 24,43))) %>%
   filter(fshort %in%  scenes_study) %>%
   group_by(name) %>%
          dplyr::summarise(avg_metric = mean(value),
                                        se = sd(value)/sqrt(length(value))) %>%
  dplyr::filter(name %in% c("Precision","Recall" ,             "F1"    ,  "Balanced Accuracy","Accuracy")) %>%
...
```  
  ...
  20180524_172142_103d 20180524_172143_103d 20180524_172144_103d 20180524_172145_103d 20180524_172326_0f51 20180524_172327_0f51 20180524_172329_0f51 20180524_172330_0f51 20180524_172331_0f51 20180524_172634_0f2d 20180524_172635_0f2d 20180524_172637_0f2d
  
```{r }
library(tidyverse)
plotch_longer_df <- read_csv(file='./data/COLORADOBASEEXT2/processed-data.csv')
...
#   theme_classic() +
#   scale_fill_manual(values=c('#999999','#E69F00'))
ggsave("figs/all_scenes_gunnison_specific-basewithextsan.png",dpi=300, dev='png',  height=8, width=10,units="in")
```

plot the studied area

```{r }
ostudyarea <- raster('data/COLORADOBASEEXT2/20180524_172637_0f2d_3B_AnalyticMS_SR_clip_merged.tif')
...
ggsave(filename = "figs/scenepreditCO_both_rev_BASEextsanjoa.png",dpi = 300,height=4,width=8,units="in")
...
"
 plotgunnib1_ostudy + plotgunnib2_ostudy + 
  ...
  caption = 'Mapbox (C)') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave("figs/co_aoi_original_ms-extsanjoa.png",dpi=300, dev='png',  height=8, width=12,units="in")
```
