---
title: "SE Bars Notebook - CH with DEMPLUS SRTM Tuol only"
...
rastlist <- list.files(path = "data/CHDEMPLUSSRTMREV/", pattern='.tif', all.files=TRUE, full.names=TRUE)
library(caret)
i=0
plo <- data.frame()
skip_to_next <- FALSE
...
write_csv(df_gunni,file='./data/CHDEMPLUSSRTMREV/processed-data.csv')
...
print(p)
ggsave("figs/all_scenes_ch_srtmtuol.png",dpi=300, dev='png',  height=8, width=10,units="in")
# Finished bar plot
...
```{r }
library(tidyverse)
plotch_longer_df <- read_csv(file='./data/CHDEMPLUSSRTMREV/processed-data.csv')
...
print(pch)
ggsave("figs/all_scenes_ch_srtmtuolonly.png",dpi=300, dev='png',  height=8, width=10,units="in")
