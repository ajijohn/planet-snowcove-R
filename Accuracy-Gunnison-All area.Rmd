...
im_gunni_site <- cc_location(extent(gunniasoandpred[[2]]), buffer = 1e5, ype = "mapbox.satellite")
#im <- cc_location(p, ype = "mapbox.terrain-rgb")

#raster::writeRaster(im_gunni_site,'data/Gunni-aoi.tiff',overwrite=TRUE)
...
```{r , echo=FALSE}
gunni_aoi <- extent(aoi_gunni_stacked)

poly_study_gunni <- as(gunni_aoi,'SpatialPolygons')
...
plotgc <- RStoolbox::ggRGB(aoi_Gunni)+
    coord_sf(crs = 4326)+
...
ggspatial::annotation_scale(location = "bl", width_hint = 0.5) +
    ggspatial::annotation_north_arrow(location = "bl", which_north = "true", 
       pad_y = unit(0.2, "mm"),style = ggspatial::north_arrow_fancy_orienteering) +
  geom_polygon(aes(long,lat), alpha = 0.2, color='green',data = poly_study_gunni) +
# coord_equal() +
  labs(x='',  y='') 
